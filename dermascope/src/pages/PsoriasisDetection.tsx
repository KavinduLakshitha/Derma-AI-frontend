...
const PsoriasisDetection = () => {
  ...
  const getPredictionMessage = (classNumber: string) => {
    switch (classNumber) {
      ...
      case '1':
        return {
          class: 'Psoriasis Detected',
          message: 'Psoriasis has been detected in the image. We recommend consulting with a dermatologist for further evaluation and treatment.',
        };
      default:
        return {
          class: 'Unknown',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/predict/psoriasis/detect', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="bg-[#5C2E0D] text-white min-h-screen">
      <header className="flex justify-between items-center py-4 px-6">
        <h1 className="text-2xl font-semibold">Psoriasis Detection & Severity Assessment</h1>
      ...
      <main className="bg-[#F6E5D3] rounded-tl-3xl rounded-tr-3xl p-8">
        ...
        >
          <div className="border-2 border-dashed border-gray-400 rounded-md p-4 text-center">
            ...
            />
            <label htmlFor="imageUpload" className="block cursor-pointer text-sm text-gray-500 mb-2">
              Choose or drag and drop an image
            </label>
            <p className="text-xs text-gray-400">Format: .jpg, .png & Max file size: 25 MB</p>
          </div>
...
export default PsoriasisDetection;
