...
const PsoriasisDetection = () => {
  ...
  const handleDetection = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/detect/psoriasis', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleSeverityAssessment = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/assess/psoriasis_severity', {
        method: 'POST',
        body: formData,
      });
    ...
    <div className="bg-[#5C2E0D] text-white min-h-screen">
      ...
      <main className="bg-[#F6E5D3] rounded-tl-3xl rounded-tr-3xl p-8">
        <h2 className="text-center text-[#5C2E0D] text-lg font-medium mt-4 mb-4">
          Upload an image for psoriasis detection and severity assessment
        </h2>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="bg-white/90 px-4 py-2 rounded-full flex items-center gap-2">
                    ...
                    <span className="text-blue-500 font-medium">
                      {scanType === 'detection' ? 'Detecting Psoriasis...' : 'Assessing Severity...'}
                    </span>
                  </div>
                </div>
              </>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      {prediction.detection.isDetected 
                        ? 'Psoriasis detected in the image' 
                        : 'No significant psoriasis detected'}
                    </p>
                  </div>
                </div>
                <div className="mt-4">
